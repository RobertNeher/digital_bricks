...
def generate_pc12():
    circuit = DigitalCircuit("12-bit Program Counter")
    X_IN, X_INC, X_MUX, X_RST, X_DFF, X_OUT = 100, 400, 700, 1000, 1200, 1500
    
    # Inputs
    d_inputs = [circuit.add_input(X_IN, 100 + i*150, f"D{i}") for i in range(12)]
    inc = circuit.add_input(X_IN, 1900, "INC")
    load = circuit.add_input(X_IN, 2050, "LOAD")
    rst = circuit.add_input(X_IN, 2200, "RESET")
    clk = circuit.add_input(X_IN, 2350, "CLK")
    
    inv_load = circuit.add_gate("inv", X_MUX - 100, 2050)
    circuit.connect(load, inv_load["inputs"][0]["id"])
    
    inv_rst = circuit.add_gate("inv", X_RST - 100, 2200)
    ...
    for i in range(12):
        ...
        # 6. Output
        out_pin = circuit.add_output(X_OUT, y + 30, f"Q{i}")
        circuit.connect(dff["outputs"][0]["id"], out_pin)
        
    return circuit.to_json()

if __name__ == "__main__":
    pc_json = generate_pc12()
    target = os.path.abspath(os.path.join(os.path.dirname(__file__), "../assets/12-bit Program Counter.json"))
