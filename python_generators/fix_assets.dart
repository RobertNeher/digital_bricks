  ...
  for (final comp in components) {
    ...
    if (oldType >= 12 || oldType == -1) {
        ...
        if (comp.containsKey('text')) {
            newType = 15; // Markdown
        } else if (comp.containsKey('internalComponents') || comp.containsKey('blueprint')) {
            newType = 16; // IC
        } 
        ...
        // Inputs (12)
        else if (inputs.isEmpty && outputs.length == 1 && (oldType == 12 || oldType == 14 || name.contains('input') || name.contains('fan'))) {
            newType = 12;
        }

        // Outputs (13)
        else if (inputs.length == 1 && outputs.isEmpty && (oldType == 13 || oldType == 15 || name.contains('output') || name.contains('fan'))) {
            newType = 13;
        }
        // Sequential Components (17-19)
        else if (comp.containsKey('storedValue') || name.contains('type') || name.contains('flip flop')) {
            if (inputs.length <= 2) {
                newType = 17; // D type
            } else if (inputs.length >= 3) {
                bool hasJK = inputs.any((p) => (p['label'] ?? "").toString().toUpperCase().contains('J'));
                if (hasJK) {
                    newType = 18;
                } else {
                    newType = 19;
        ...
        // Button (14)
        else if (name.contains('button') || (oldType == 14 && inputs.isEmpty && outputs.isEmpty)) {
            newType = 14;
        }
    }
